...
public class A_Knapsack {
    ...
    int getMaxWeight(InputStream stream) {
        ...
        // dp[w] = максимальный вес, который можно набрать для вместимости w
        int[] dp = new int[W + 1];
        dp[0] = 0;  // для пустого рюкзака вес 0

        // Заполняем для всех вместимостей от 1 до W
        for (int w = 1; w <= W; w++) {
            int max = 0;
            // Пробуем каждый слиток
            for (int i = 0; i < n; i++) {
                if (gold[i] <= w) {
                    // Если слиток влезает, смотрим: кладем его + оптимально заполняем остаток
