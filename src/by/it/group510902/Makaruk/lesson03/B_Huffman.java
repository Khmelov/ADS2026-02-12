...
public class B_Huffman {
    ...
    String decode(InputStream inputStream) throws FileNotFoundException {
        ...
        // Map to store code -> character mapping
        Map<String, Character> codeToChar = new HashMap<>();

        // Read k lines with character codes
        ...
        // Decode the string by reading bits until we find a matching code
        StringBuilder currentCode = new StringBuilder();
        for (int i = 0; i < encoded.length(); i++) {
            currentCode.append(encoded.charAt(i));

            // Check if currentCode is a valid code in our map
            if (codeToChar.containsKey(currentCode.toString())) {
                // Found a match, append the corresponding character
